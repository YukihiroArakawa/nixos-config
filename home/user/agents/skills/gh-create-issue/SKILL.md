...
description: Create a GitHub issue with a Conventional Commit-style title and the repository's task issue template by using the gh CLI. Use when the user asks to create, file, or open an issue for planned work in the current repository, especially when they want the issue written in English or based on .github/ISSUE_TEMPLATE/task.md.
---
...
4. Draft a concise English title and body. Format the title as `<type>(<scope>): <description>`, following Conventional Commits. Choose the type by intent, such as `fix` for correcting behavior, `feat` for a new capability, `refactor` for restructuring, `docs` for documentation, or `chore` for maintenance. Use a short lowercase scope that identifies the affected area, write the description in the imperative mood, and keep the full title under 72 characters where practical. Do not invent requirements; leave genuinely unknown details explicit.
